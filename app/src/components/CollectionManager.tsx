    ...
    return (
        <div className="w-full max-w-5xl mx-auto space-y-6">
            <div className="rounded-2xl border border-[#2E2E2E] bg-[#232323] p-6 shadow-xl">
                ...
                <p className="text-sm text-vex-muted">Group playlists and pin favorites.</p>
                    ...
                    {collections.length === 0 ? (
                        ...
                        collections.map((collection) => (
                            <div key={collection.id} className="rounded-xl border border-[#2E2E2E] bg-[#171717] p-4">
                                ...
                                <p className="text-xs text-vex-muted">{collection.playlistIds.length} playlists</p>
                            </div>
                        ))
                    )}
                </div>
                        ...
                        playlists.map((playlist) => (
                            ...
                            >
                                <div>
                                    ...
                                    <p className="text-xs text-vex-muted">{playlist.collectionId ? `Collection: ${collections.find((c) => c.id === playlist.collectionId)?.name ?? 'Unknown'}` : 'Unassigned'}</p>
                                </div>
                                <div className="flex items-center gap-3">
                                    <button
                                        onClick={() => onToggleFavorite(playlist.id)}
                                        className={`rounded-full border px-3 py-1 text-xs font-semibold ${playlist.isFavorite ? 'border-vex-primary bg-vex-primary/10 text-vex-primary' : 'border-[#2E2E2E] text-gray-300'}`}
                                    >
