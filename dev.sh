...
# Check dependencies
if [ ! -d "experiments/fullstack/web/node_modules" ]; then
    echo "📦 node_modules missing. Running npm install..."
    (cd experiments/fullstack/web && npm install)
fi
...
if command -v bundle >/dev/null 2>&1; then
    cd experiments/fullstack/web/backend && bundle exec rackup -p 4567 &
else
    echo "⚠️ bundle command not found. Trying to run without bundle exec..."
    cd experiments/fullstack/web/backend && rackup -p 4567 &
fi

# Start the Frontend (Next.js)
echo "⚛️ Starting Next.js Frontend on port 3000..."
cd experiments/fullstack/web && npm run dev &
